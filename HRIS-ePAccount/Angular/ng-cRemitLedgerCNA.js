...
*/
ng_HRD_App.controller("cRemitLedgerCNA_ctrlr", function ($scope, $compile, $http, $filter) {
    var s = $scope
    var h = $http
    s.rowLen = "10"
    s.datalistgrid = null;
    ...
    function init() {
        ...
        h.post("../cRemitLedgerCNA/InitializeData",
            {
    ...
    var getLetterList = function () {
        h.post("../cRemitLedgerCNA/LetterList",
            {
        ...
        s.oTable = $('#datalist_grid').dataTable(
            {
                ...
                columns: [
                    ...
                    {
                        "mData": "peucv_dues", "mRender": function (data, type, full, row) {
                    ...
                    {
                        "mData": "mutuary_amt", "mRender": function (data, type, full, row) {
                            var payroll_amount = data
    ...
    s.btn_add_click = function () {
        ...
        s.ADDEDITMODE = "ADD";
        h.post("../cRemitLedgerCNA/GetVoucher",
            {
                p_batch_nbr: s.ddl_batch_nbr
            }
        ).then(function (d) {
    ...
    s.btn_edit_action = function (par_row_index) {
        ...
        s.show_in_edit = true;
        s.ADDEDITMODE = "EDIT";
        s.ModalTitle = "View C.N.A Remittance Data";
        //h.post("../cRemitLedgerCNA/GetVoucher",
        //    {
        //        p_batch_nbr: s.ddl_batch_nbr
        //    }).then(function (d) {
        ...
        s.amount_decimal = parseFloat(s.datalistgrid[par_row_index].peucv_dues).toFixed(2);
        s.txtb_peucv_amount = s.amount_decimal;
        s.amount_decimal1 = parseFloat(s.datalistgrid[par_row_index].mutuary_amt).toFixed(2);
        s.txtb_mortuary_amount = s.amount_decimal1;
    ...
    s.btn_delete_row = function (row_index) {
        ...
        swal({
            ...
            .then(function (willDelete) {
                if (willDelete) {
                    h.post("../cRemitLedgerCNA/DeleteCNADetails", {
                        data: dt
                    }).then(function (d) {
    ...
    s.btn_view_report = function (year, month) {
        ...
        h.post("../cRemitLedgerCNA/PrintCNAMonthly").then(function (d) {
            if (d.data.message == "success") {

                
                var employment_type_rep = ""
                if ($("#txtb_remittance_type").val() == "Casual Employees") {

                    employment_type_rep = "CE"
                }
                else if ($("#txtb_remittance_type").val() == "Regular Employees")
                {
                    employment_type_rep = "RE"
                }
                else
                {
                    employment_type_rep = "JO"
                }
                ...
                var ReportPath = "~/Reports/cryRemittanceOTHERS1/cryRemittanceCNAMonthlyRecon.rpt"
                var sp = "sp_monthly_remittance_cna_rep"
                var parameters = "p_remittancetype_code," + "15" + ",p_employment_type," + employment_type_rep + ",p_remit_year," + year + ",p_remit_month," + month + ",p_remittance_ctrl_nbr," + s.txtb_remittance_ctrl_nbr
    ...
    s.ddl_department_selected_change = function (par_value) {
        ...
        if ((s.ddl_lastname_letter != "" || s.ddl_departments != "") && s.ddl_batch_nbr != "") {
            h.post("../cRemitLedgerCNA/RetrieveListGrid",
                {
                    p_department_code: s.ddl_departments,
                    p_starts_letter: s.ddl_lastname_letter,
                    p_batch_nbr: s.ddl_batch_nbr
    ...
    s.set_voucher_index = function (par_voucher_index) {
        if (par_voucher_index != "") {
            ...
            h.post("../cRemitLedgerCNA/RetrieveEmployees", {
    ...
    s.btn_save_click = function () {
        if (ValidateFields()) {
            ...
            if (s.ADDEDITMODE == "ADD") {
                ...
                h.post("../cRemitLedgerCNA/SaveADDSSSInDatabase", { data: data }).then(function (d) {
                    if (d.data.message == "success") {
                ...
                h.post("../cRemitLedgerCNA/SaveEDITSSSInDatabase", { data: data }).then(function (d) {
                    if (d.data.message == "success") {
                        //data.employee_name  = $("#ddl_employee option:selected").html();
                        //data.sss_nbr        = s.txtb_sss_number;
                        //data.payroll_year   = s.txtb_payroll_year;
    ...
    s.btn_individual_print = function (row_id) {
        h.post("../cRemitLedgerCNA/PrintBack").then(function (d) {
    ...
    s.ExctractToExcel = function (sq_m) {
        ...
        h.post("../cRemitLedgerCNA/ExctractToExcelSSS",
            {
    ...
    s.btn_view_grand_total = function () {
        h.post("../cRemitLedgerCNA/RetrieveGrandTotal",
            {
