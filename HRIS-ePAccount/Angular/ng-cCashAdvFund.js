ng_HRD_App.controller("cCashAdvFund_ctrlr", function ($scope, $compile, $http, $filter) {
    var s = $scope
    var h = $http
    ...
    function init() {
        ...
        //**********************************************
        h.post("../cCashAdvFund/initializeData").then(function (d) {
            if (d.data.message == "success") {
                userid = d.data.userid;
              

                if (d.data.caFundLst.length > 0) {
                    init_table_data(d.data.caFundLst);
                }
                else {
                    init_table_data([]);
                }
        ...
        s.oTable = $('#datalist_grid').dataTable(
            {
                ...
                pageLength: 10,
                columns: [
                    { "mData": "cafund_code", "mRender": function (data, type, full, row) { return "<span class='text-center btn-block'>" + data + "</span>" } },
                    { "mData": "cafund_descr", "mRender": function (data, type, full, row) { return "<span class='text-left btn-block'>" + data + "</span>" } },
                    {
                        "mData": null,
                        "bSortable": false,
        ...
        setTimeout(function () {
            h.post("../cCashAdvFund/initializeData").then(function (d) {
                if (d.data.message == "success") {
                    ...
                    h.post("../cCashAdvFund/GetLasCode").then(function (d) {
                        if (d.data.message == "success") {
                            var nextCode = "";
                            var ids = d.data.ids;
    ...
    s.btn_save_click = function () {
        if (ValidateFields()) {
            h.post("../cCashAdvFund/CheckExist", {
                cafund_code: s.txtb_code
            }).then(function (d) {
                if (d.data.message == "success") {
                    ...
                    var data = {
                        cafund_code     : s.txtb_code
                        , cafund_descr  : s.txtb_description
                    }

                    
                    h.post("../cCashAdvFund/SaveCAFund", { data: data }).then(function (d) {
                        if (d.data.message == "success") {
    ...
    s.btn_edit_action = function (row_id) {
        h.post("../cCashAdvFund/CheckExist2", {
            cafund_code: s.datalistgrid[row_id].cafund_code
        }).then(function (d) {
            if (d.data.message == "success") {
                h.post("../cCashAdvFund/GetData", {
                    cafund_code: s.datalistgrid[row_id].cafund_code
                }).then(function (d) {
                    if (d.data.message == "success") {
                        ...
                        s.txtb_code         = s.datalistgrid[row_id].cafund_code;
                        s.txtb_description  = s.datalistgrid[row_id].cafund_descr;

                        $('#edit').attr('ngx-data', row_id);
                        var row_edited = $('#edit').attr("ngx-data");
                        s.datalistgrid[row_edited].cafund_code  = s.txtb_code;
                        s.datalistgrid[row_edited].cafund_descr = s.txtb_description;
                    }
                });
            }
            else {
    ...
    s.SaveEdit = function () {
        if (ValidateFields()) {
            ...
            h.post("../cCashAdvFund/SaveEditCAFund", { data: data }).then(function (d) {
                if (d.data.message == "success") {
                    var row_edited = $('#edit').attr("ngx-data");
                    s.datalistgrid[row_edited].cafund_code  = s.txtb_code;
                    s.datalistgrid[row_edited].cafund_descr = s.txtb_description
    ...
    s.btn_del_row = function (row_index) {
        swal({
            ...
            .then(function (willDelete) {
                if (willDelete) {
                    h.post("../cCashAdvFund/DeleteCAFund", {
                        cafund_code: s.datalistgrid[row_index].cafund_code
                    }).then(function (d) {
                        if (d.data.message == "success") {
