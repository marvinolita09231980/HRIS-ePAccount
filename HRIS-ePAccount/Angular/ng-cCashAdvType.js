ng_HRD_App.controller("cCashAdvType_ctrlr", function ($scope, $compile, $http, $filter) {
    var s = $scope
    var h = $http
    ...
    function init() {
        ...
        //**********************************************
        h.post("../cCashAdvType/initializeData").then(function (d) {
            if (d.data.message == "success") {
                userid = d.data.userid;
              
                if (d.data.caTypeLst.length > 0) {
                    init_table_data(d.data.caTypeLst);
                }
                else {
                    init_table_data([]);
                }
        ...
        s.oTable = $('#datalist_grid').dataTable(
            {
                ...
                pageLength: 10,
                columns: [
                    { "mData": "catype_code", "mRender": function (data, type, full, row) { return "<span class='text-center btn-block'>" + data + "</span>" } },
                    { "mData": "catype_descr", "mRender": function (data, type, full, row) { return "<span class='text-left btn-block'>" + data + "</span>" } },
                    {
                        "mData": null,
                        "bSortable": false,
        ...
        setTimeout(function () {
            h.post("../cCashAdvType/initializeData").then(function (d) {
                if (d.data.message == "success") {
                    ...
                    s.datalistgrid = d.data.caTypeLst
                    if (d.data.caTypeLst.length > 0) {
                        s.oTable.fnAddData(d.data.caTypeLst);
                    }
                }
                else {
                    swal(d.data.message, { icon: "warning", });
    ...
    s.btn_save_click = function () {
        if (ValidateFields()) {
            h.post("../cCashAdvType/CheckExist", {
                catype_code: s.txtb_code
            }).then(function (d) {
                if (d.data.message == "success") {
                    ...
                    var data = {
                        catype_code: s.txtb_code
                        , catype_descr: s.txtb_description
                    }

                  
                    h.post("../cCashAdvType/SaveCAType", { data: data }).then(function (d) {
                        if (d.data.message == "success") {
    ...
    s.btn_edit_action = function (row_id) {
        h.post("../cCashAdvType/CheckExist2", {
            catype_code: s.datalistgrid[row_id].catype_code
        }).then(function (d) {
            if (d.data.message == "success") {
                h.post("../cCashAdvType/GetData", {
                    catype_code: s.datalistgrid[row_id].catype_code
                }).then(function (d) {
                    if (d.data.message == "success") {
                        ...
                        s.txtb_code = s.datalistgrid[row_id].catype_code;
                        s.txtb_description = s.datalistgrid[row_id].catype_descr;
                        
                        $('#edit').attr('ngx-data', row_id);
                        var row_edited = $('#edit').attr("ngx-data");
                        s.datalistgrid[row_edited].catype_code = s.txtb_code;
                        s.datalistgrid[row_edited].catype_descr = s.txtb_description;
                    }
                });
            }
            else {
    ...
    s.SaveEdit = function () {
        if (ValidateFields()) {
            ...
            h.post("../cCashAdvType/SaveEditCAType", { data: data }).then(function (d) {
                if (d.data.message == "success") {
                    var row_edited = $('#edit').attr("ngx-data");
                    s.datalistgrid[row_edited].catype_code = s.txtb_code;
                    s.datalistgrid[row_edited].catype_descr = s.txtb_description
    ...
    s.btn_del_row = function (row_index) {
        swal({
            ...
            .then(function (willDelete) {
                if (willDelete) {
                    h.post("../cCashAdvType/DeleteCAType", {
                        catype_code : s.datalistgrid[row_index].catype_code
                    }).then(function (d) {
                        if (d.data.message == "success") {
