...
ng_HRD_App.controller("cRemitLedgerSSS_ctrlr", function ($scope, $compile, $http, $filter) {
    ...
    function init() {
        ...
        h.post("../cRemitLedgerSSS/InitializeData",
        {
    ...
    var getLetterList = function () {
        h.post("../cRemitLedgerSSS/LetterList",
            {
                ...
                columns: [
                    ...
                    },
                    {
                        "mData": "payroll_amount", "mRender": function (data, type, full, row)
                        {
                            ...
                            if (payroll_amount < 1950) {
                                payroll_amount = payroll_amount + 10
                            }

                            else if (payroll_amount >= 1950)
                            {
                                payroll_amount = payroll_amount + 30
                            }
                    ...
                    },
                    {
                        ...
                        "mRender": function (data, type, full, row) {
                            ...
                            return '<center><div class="btn-group">' +
                                ...
                                '<button type="button" class="btn btn-primary btn-sm" ng-click="btn_individual_print(' + row["row"] + ')" data-toggle="tooltip" data-placement="top" title="Print"><i class="fa fa-print"></i></button>' +
                                   '</div></center>';
                        }
                    }
    ...
    s.search_in_list_letter = function (value, table) {
        ...
        $("#" + table).DataTable().column(7).search(value).draw();
    ...
    s.btn_edit_action = function (par_row_index)
    {
        ...
        s.show_in_edit = true;
        s.ADDEDITMODE   = "EDIT";
        s.ModalTitle    = "Edit SSS Remittance Data";
        h.post("../cRemitLedgerSSS/GetVoucher",
            {
                p_batch_nbr: s.ddl_batch_nbr
            }).then(function (d)
            {
                ...
                s.txtb_sss_amount       = s.amount_decimal;
    ...
    s.btn_delete_row = function (row_index)
    {
        ...
        swal({
            ...
            {
                if (willDelete) {
                    h.post("../cRemitLedgerSSS/DeleteSSSDetails", {
                        data: dt
                    }).then(function (d) {
    ...
    s.ddl_department_selected_change = function (par_value)
    {
        ...
        if ((s.ddl_lastname_letter != "" || s.ddl_departments != "") && s.ddl_batch_nbr !="") {
            h.post("../cRemitLedgerSSS/RetrieveListGrid",
                {
                    p_department_code:  s.ddl_departments,
                    p_starts_letter:    s.ddl_lastname_letter,
                    p_batch_nbr:        s.ddl_batch_nbr
    ...
    s.set_voucher_index = function (par_voucher_index)
    {
        if (par_voucher_index != "")
        {
            ...
            h.post("../cRemitLedgerSSS/RetrieveEmployees", {
    ...
    s.btn_save_click = function ()
    {
        if (ValidateFields())
        {
            ...
            if (s.ADDEDITMODE == "ADD") {
                ...
                h.post("../cRemitLedgerSSS/SaveADDSSSInDatabase", { data: data }).then(function (d)
                {
                    if (d.data.message == "success")
                    {
            ...
            else if (s.ADDEDITMODE == "EDIT")
            {
                ...
                h.post("../cRemitLedgerSSS/SaveEDITSSSInDatabase", { data: data }).then(function (d) {
                    if (d.data.message == "success")
                    {
                        //data.employee_name  = $("#ddl_employee option:selected").html();
                        //data.sss_nbr        = s.txtb_sss_number;
    ...
    {
        h.post("../cRemitLedgerSSS/PrintBack").then(function (d) {
    ...
    {
        h.post("../cRemitLedgerSSS/RetrieveGrandTotal",
            {
