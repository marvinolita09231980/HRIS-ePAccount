...
{
    public class cRemitCertController : Controller
    {
        ...
        public ActionResult InitializeData(string p_remitType_code, string p_empl_type, string p_dept_code, string p_letter)
        {
            try
            {
                ...
                if (Session["PreviousValuesonPage_cRemitCert"] == null || Session["PreviousValuesonPage_cRemitCert"].ToString() == "")
                {
                    ...
                    return JSON(new { prevValues, userid, listgrid, remittancetypelist, department_list, emplType, allowAdd, allowDelete, allowEdit, allowPrint, allowView }, JsonRequestBehavior.AllowGet);
                }
                else
                {
                    string[] prevValues = Session["PreviousValuesonPage_cRemitCert"].ToString().Split(new char[] { ',' });
        ...
        public ActionResult Filter(string p_remit_type, string p_empl_type, string p_letter, string p_dept)
        {
            try
            {
                db_pacco.Database.CommandTimeout = int.MaxValue;
                if (p_remit_type == "" || p_empl_type == "" || p_dept == "")
                {
                    var listgridFiltered = db_pacco.sp_remittance_cert_tbl_list1("", "", "", "").ToList();
                    return JSON(new { listgridFiltered }, JsonRequestBehavior.AllowGet);
                }
                else
                {
                    var listgridFiltered = db_pacco.sp_remittance_cert_tbl_list1(p_remit_type, p_empl_type, p_dept, p_letter).ToList();
                    return JSON(new { listgridFiltered }, JsonRequestBehavior.AllowGet);
                }
        ...
        public ActionResult PreviousValuesonPage_cRemitCert
            (
            ...
            , string par_letter
            , string par_remit_desc
            , string par_empl_type_decr
            , string par_dept_descr
            , string par_show_entries
            , string par_page_nbr
            )
        {
            var PreviousValuesonPage_cRemitCert = par_remittancetype_code
                                                + "," + par_empl_type
                                                + "," + par_dept_code
                                                + "," + par_empl_id
                                                + "," + par_empl_name
            ...
            Session["PreviousValuesonPage_cRemitCert"] = PreviousValuesonPage_cRemitCert;
            return JSON(PreviousValuesonPage_cRemitCert, JsonRequestBehavior.AllowGet);
        }
        ////*********************************************************************//
