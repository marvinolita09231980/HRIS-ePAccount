...
{
    public class cPayRegistrySalaryRegularController : Controller
    {
        ...
        public ActionResult InitializeData(string par_empType)
        {
            ...
            string ddl_department   = PreviousValuesonPage_cPayRegistry[7].ToString().Trim();

            var empType = db_pacco.vw_employmenttypes_tbl_list.Where(a => a.employment_type == ddl_emp_type).ToList();
            //var sp_payrollregistryaccounting_goupings_tbl_list = db_pacco.sp_payrollregistryaccounting_goupings_tbl_list(ddl_year, ddl_month, ddl_template).ToList();
            ...
            var department_list         = db_pacco.vw_departments_tbl_list.ToList();
            var sub_department_list     = db_pacco.vw_subdepartments_tbl_list.ToList();
            var division_list           = db_pacco.vw_divisions_tbl_list.ToList();
            var section_list            = db_pacco.vw_sections_tbl_list.ToList();

            var sp_payrollregistryaccounting_dtl_tbl_list = db_pacco.sp_payrollregistryaccounting_dtl_tbl_list(ddl_year, ddl_month, ddl_registry_nbr, ddl_template, ddl_group, ddl_emp_type).ToList();
            ...
            return Json(new { empType, userid,ddl_year, ddl_month, ddl_emp_type, ddl_template, ddl_group, payroll_template, payrolltype, sp_payrollregistryaccounting_dtl_tbl_list, department_list, sub_department_list, division_list, section_list }, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult SelectPayrollGroup(string par_ddl_group, string par_registry_nbr)
        {
            try
            {
                ...
                var sp_payrollregistryaccounting_dtl_tbl_list = db_pacco.sp_payrollregistryaccounting_dtl_tbl_list(ddl_year, ddl_month, ddl_registry_nbr, ddl_template,par_ddl_group,ddl_emp_type).ToList();
                
                return Json(new { sp_payrollregistryaccounting_dtl_tbl_list }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
