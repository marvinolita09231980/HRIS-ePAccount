...
// AUTHOR                    DATE            PURPOSE
//----------------------------------------------------------------------------------
// Jorge Rusom Villanueva       10/21/2019      Code Creation
//**********************************************************************************
...
namespace HRIS_ePAccount.Controllers
{
    public class cRemitLedgerCNAController : Controller
    {
        ...
        public ActionResult InitializeData(string p_department_code, string p_starts_letter, int p_batch_nbr)
        {
            ...
            var listgrid = db_pacco.sp_remittance_ledger_info_CNA(remittance_ctrl_nbr, p_department_code, p_starts_letter, p_batch_nbr, "", "").ToList();
        ...
        //*********************************************************************//
        public ActionResult DeleteCNADetails(remittance_dtl_cna_tbl data)
        {

            try
            {
                db_pacco.Database.CommandTimeout = int.MaxValue;
                var dt = db_pacco.remittance_dtl_cna_tbl.Where(a =>
                    a.remittance_ctrl_nbr == data.remittance_ctrl_nbr &&
                    a.empl_id == data.empl_id &&
                    a.voucher_nbr == data.voucher_nbr
                    ).FirstOrDefault();

                db_pacco.remittance_dtl_cna_tbl.Remove(dt);
                db_pacco.SaveChanges();
                return JSON(new { message = "success" }, JsonRequestBehavior.AllowGet);
            }
            catch (DbEntityValidationException e)
        ...
        //*********************************************************************//
        public ActionResult PrintCNAMonthly()
        {
